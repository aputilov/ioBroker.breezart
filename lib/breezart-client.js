...
class BreezartClient {
  ...
  */
    ...
    this.getProperties(() => {
      ...
      this.makeRequest(requestType, null, (error, response) => {
        if (error) {
          ...
          this.OldSpeed = this.Speed //АП: Для автоматизации запоминаем старое значение
  ...
  setRotationSpeed (speed, callback) {
    ...
    if (speed > 0 && speed < 9) {
      this.getStatus(() => {
        console.log('Получаем статус....')
      
        let requestType = BreezartClient.RequestPrefix.CHANGE_FAN_SPEED;
        let data = speed; //Предположим что скорость правильно передается
        console.log('Set speed at: ', data);
